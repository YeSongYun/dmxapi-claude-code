...
- **预设模型新增两款支持超长上下文的高速模型：glm-5.3-flash-cc 和 qwen3.8-flash-cc**
  这两款新模型都支持高达 100 万 token 的超长上下文，适合处理篇幅很长的文档或对话。添加后，它们会自动带上"[1m]"标记，方便你识别其超长上下文能力。无需手动操作，在模型选择菜单中找到它们即可直接使用，其中 glm-5.3-flash-cc 排在 glm-5.3-cc 之后，qwen3.8-flash-cc 排在 qwen3.8-max-cc 之后。

### 📖 文档

- 更新了上一个版本的更新日志文档，对使用没有影响。

### 🔨 杂项

- 发布了 v1.9.5 版本，主要内容就是上述两款新的预设模型。
...
- **更新日志整理**
  例行更新了上一版本的更新日志文档，对使用者没有直接影响，无需任何操作。

### 🔨 杂项

- **AI 润色功能支持更长的输出内容**
  本次发布将 AI 润色功能允许生成的内容长度上限从约 4 千字提升到了约 3.2 万字。以前润色较长文章时可能出现输出被截断、内容不完整的情况，现在可以获得更完整的润色结果。此改进对各家 AI 服务（包括 Anthropic、OpenAI 及 Gemini 等）同时生效，您无需进行任何设置，升级到本版本后自动生效。

## 1.9.3 (2026-08-27)

### 🐛 问题修复

- **修复了自动撰写更新日志的功能从未生效的问题**
  之前系统靠模型名称的开头判断该用哪家的接口，结果判断错了，请求被发到了一个早已停用的旧接口上，导致每次发布的更新日志都没有被智能整理过。修复后，今后发布的更新说明会按预期呈现为更易读的整理版本。此修复自动生效，您无需做任何操作。

- **修正了更新日志撰写规则中的几处缺陷，并补写了上一版本的更新说明**
  发布后的检查确认之前几个版本的更新日志都退回成了原始的提交记录。本次修正了相关规则，避免以后再次出现这种情况。此改动不影响您的日常使用。

### 📖 文档

- 补充更新了上一版本（v1.9.2）的更新日志内容。

### 🔧 CI/CD

- 改进了发布流程：自动整理更新日志失败时会明确报警，不再静默跳过；同时修正了内部版本号记录的维护方式。此改动不影响您的日常使用。

### 🔨 杂项

- 完成了 v1.9.3 版本的发布准备工作（同步各处的版本号）。此改动不影响您的日常使用。
