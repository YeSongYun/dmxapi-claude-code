...
正确的配置前缀 `claudeCode` 由扩展 `package.json` 中 `contributes.configuration.id` 字段定义，与扩展 ID 的连字符形式（`claude-code`）无关，并非 VSCode 平台自动转换。工具从 VSCode 功能引入之初就使用了错误键名，导致：
...
- 不主动清理 settings.json 中残留的旧键 `claude-code.environmentVariables`：VSCode 扩展不读取旧键，故旧键残留无实际副作用；双键共存时仅旧键数据不再更新，后续版本视需求再评估迁移
- 不修改写入数据结构（字段名、格式等）
- 不调整其他 VSCode 相关逻辑
...
| `TestIsVSCodeConfigured` - 含新键 | 不存在 | 新增：含 `claudeCode.environmentVariables` → 返回 true |
| `TestIsVSCodeConfigured` - 含旧键 | 含旧键返回 true | 保留：含 `claude-code.environmentVariables` → 仍返回 true |
| `TestIsVSCodeConfigured` - 新旧键共存 | 不存在 | 新增：同时含两键 → 返回 true（现有逻辑自然覆盖） |
| `TestIsVSCodeConfigured` - 旧键值为空数组 | 含旧键（含空数组）返回 true | 行为不变：键存在即视为已配置，不校验值内容 |
